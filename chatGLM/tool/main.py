...
def run_tool(tools, llm, prompt_chain: List[str]):
    ...
    for prompt in prompt_chain:
         agent_info = agent.run(prompt)
    return agent_info

def search(prompt):
    query  = run_tool([Extractor()], llm, [prompt,])
    extract_tool = Extractor()
    extract_info = extract_tool.run(query)
    print(query , extract_info)
    return extract_info
if __name__ == "__main__":
    ...
    #Extractor: 单个工具调用
    prompt = """
主题:维多利亚的秘密Victoria's Secret内衣
品牌卖点:软细腻，隐形效果好，质感高，平价实惠，光面无痕
受众群体:适合学生党和上班族
请你根据以上内容以最快速度提取出主要信息,只需提取到以下内容：营销产品品类、产品品牌、品牌卖点、受众群体，以json格式返回，json格式为{"category": ,"brand": ,"feature": ,"crowd":}
"""
    search(prompt)
    ...
    #    "根号2加上根号三等于多少？",
    #]),
    #print(f"time usage:{time.time() - tik}s")
